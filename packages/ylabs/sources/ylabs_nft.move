module momentx::ylabs_nft {
    ...
    struct Global has key, store {
        ...
        nfts: Table<ID, YlabsNFTConfig>,
        url_init: String,
        url_redeemed: String,
    }
    struct YlabsNFTConfig has store {
        merchant_white_list: VecSet<address>,
        merchant_redeemed: Option<address>,
    }

    struct YlabsNFT has key, store {
        id: UID,
        name: String,
        description: String,
        url: String,
    ...
    public entry fun airdrop(
        ...
        let ylabs_nft_config = YlabsNFTConfig {
            merchant_white_list: vec_set::empty(),
            merchant_redeemed: none(),
        };
        table::add(&mut global.nfts, object::id(&nft), ylabs_nft_config);
        transfer(nft, to)
    }
